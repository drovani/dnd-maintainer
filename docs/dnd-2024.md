...
- Every background grants exactly: one `asi` grant (3 points distributed among a named subset of abilities), two skill proficiencies, one tool proficiency (sometimes a choice), and one Origin feat via `{ type: 'feat', featId: '...' }`. Backgrounds do **not** grant languages in the 2024 ruleset — every character knows Common plus two languages of choice, granted by their species.
