...
export function reconstructBuild(
  ...
  const levelRows = [...rows.filter((r): r is LevelRow => !isCreationRow(r))].sort((a, b) => a.sequence - b.sequence);
  ...
  if (creationRow.base_abilities) {
    ...
    if (!parsed.success) {
      throw new Error(`Invalid base_abilities in creation row: ${parsed.error.message}`);
    }
