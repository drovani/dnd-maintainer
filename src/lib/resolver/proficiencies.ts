...
export function resolveSkills(
  ...
  for (const skill of DND_SKILLS) {
    ...
    for (const { grant } of abilityCheckBonuses) {
      ...
      const bonusValue = grant.value === 'half-proficiency' ? Math.ceil(proficiencyBonus / 2) : 0;
      if (bonusValue > 0) {
...
export function resolveProficiencies(
  ...
  for (const { grant, source } of collectGrantsByType(bundles, 'proficiency-choice')) {
    switch (grant.category) {
      ...
      default: {
        throw new Error(
          `Unhandled proficiency choice category: ${(grant satisfies never as { category: string }).category}`
        );
      }
    }
  }
