...
const DEFAULT_ASSIGNMENTS: Record<keyof AbilityScores, number | null> = {
  str: null,
  dex: null,
  con: null,
  int: null,
...
export function AbilitiesStep() {
  ...
  const baseAbilities: AbilityScores = creationRow?.base_abilities ?? DEFAULT_ABILITIES;
  ...
  const [abilityAssignments, setAbilityAssignments] = useState<Record<keyof AbilityScores, number | null>>(() => {
    ...
    const baseSorted = Object.values(baseAbilities)
      .slice()
      .sort((a, b) => b - a);
    const standardSorted = [...STANDARD_ARRAY].sort((a, b) => b - a);
    const isCompletePermutation =
      baseSorted.length === standardSorted.length && baseSorted.every((v, i) => v === standardSorted[i]);
    ...
    const assignments: Record<keyof AbilityScores, number | null> = {
      str: null,
      dex: null,
      con: null,
      int: null,
  ...
  const assignAbilityScore = (ability: keyof AbilityScores, value: number | null) => {
    ...
    if (value !== null) {
      ...
      const assignedCount = Object.entries(newAssignments).filter(([, val]) => val === value).length;
  ...
  const unassignValue = (value: number) => {
    const abilityToUnassign = Object.entries(abilityAssignments).find(([, val]) => val === value)?.[0];
  ...
  const handleRollScores = useCallback(() => {
    ...
    rollIntervalRef.current = setInterval(() => {
      ...
      if (ticks >= totalTicks) {
        ...
        const resetAssignments: Record<keyof AbilityScores, number | null> = {
          str: null,
          dex: null,
          con: null,
          int: null,
          ...
          {options.map((v) => (
            <SelectItem key={v} value={String(v)}>
              <span className="flex items-center justify-between w-full">
                {v}
                {assignedByOthers.has(v) && v !== currentValue && <Check className="size-3 text-muted-foreground" />}
              </span>
            </SelectItem>
          ))}
        </SelectContent>
      ...
      <Tabs value={abilityMethod} onValueChange={handleMethodChange}>
        <TabsList className="w-full">
          <TabsTrigger value="standard-array" className="flex-1">
            {tc('characterBuilder.abilities.standardArray')}
          </TabsTrigger>
          <TabsTrigger value="point-buy" className="flex-1">
            {tc('characterBuilder.abilities.pointBuy')}
          </TabsTrigger>
          <TabsTrigger value="rolling" className="flex-1">
            {tc('characterBuilder.abilities.rolling')}
          </TabsTrigger>
        </TabsList>

        <TabsContent value="standard-array" className="space-y-3 mt-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-muted-foreground text-sm">{tc('characterBuilder.abilities.assignEachValue')}</p>
        ...
        <TabsContent value="point-buy" className="space-y-3 mt-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-muted-foreground text-sm">{tc('characterBuilder.abilities.spendPoints')}</p>
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-muted-foreground">
                {tc('characterBuilder.abilities.points')}
              </span>
              <span
                className={`text-2xl font-bold tabular-nums ${pointsRemaining === 0 ? 'text-muted-foreground' : pointsRemaining < 5 ? 'text-amber-600' : 'text-foreground'}`}
              >
                {pointsRemaining}
              </span>
              ...
              return renderAbilityCard(
                ...
                <div className="flex items-center gap-1">
                  <Button
                    ...
                    title={
                      canDecrement
                        ? tc('characterBuilder.abilities.decreaseScore', { count: decReturn })
                        : tc('characterBuilder.abilities.minimumScore')
                    }
                  >
                  ...
                  <Button
                    ...
                    title={
                      canIncrement
                        ? tc('characterBuilder.abilities.increaseScore', { count: incCost })
                        : score >= 15
                          ? tc('characterBuilder.abilities.maximumScore')
                          : tc('characterBuilder.abilities.notEnoughPoints')
                    }
                  >
        ...
        <TabsContent value="rolling" className="space-y-3 mt-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="text-muted-foreground text-sm">{tc('characterBuilder.abilities.rollInstructions')}</p>
            <div className="flex items-center gap-3">
              {pointBuyDiff !== null && !isRolling && (
                <div
                  className={`flex items-center gap-1 text-sm font-medium ${pointBuyDiff > 0 ? 'text-green-600' : pointBuyDiff < 0 ? 'text-red-600' : 'text-muted-foreground'}`}
                >
                  {pointBuyDiff > 0 ? (
                  ...
                  <span>
                    {pointBuyDiff > 0
                      ...
                      : pointBuyDiff < 0
                        ? `${pointBuyDiff}`
                        : tc('characterBuilder.abilities.even')}{' '}
              ...
              renderAbilityCard(
                ...
                rolledValues.length > 0 && !isRolling ? (
                  ...
                  <span className="text-xs text-muted-foreground w-16 text-center">
                    {isRolling ? '...' : tc('characterBuilder.abilities.rollFirst')}
                  </span>
                )
              )
            )}
