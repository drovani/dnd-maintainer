              ...
              >
                <Checkbox
                  ...
                  onCheckedChange={(checked) => {
                    const next = checked ? [...current, skillId] : current.filter((s) => s !== skillId);
                    if (next.length === 0) {
            ...
            return (
              ...
              >
                <Checkbox
                  ...
                  disabled={isDisabled}
                  onCheckedChange={(checked) => {
                    const next = checked ? [...current, langId] : current.filter((l) => l !== langId);
                    if (next.length === 0) {
                      onClear(choice.choiceKey);
                    } else {
                ...
                />
                <Label htmlFor={`choice-lang-${choice.choiceKey}-${langId}`} className="flex-1 cursor-pointer">
                  {t(`languages.${langId}`)}
                </Label>
              </div>
            );
            ...
            return (
              ...
              >
                <Checkbox
                  ...
                  disabled={isDisabled}
                  onCheckedChange={(checked) => {
                    const next = checked ? [...current, toolId] : current.filter((tool) => tool !== toolId);
                    if (next.length === 0) {
                      onClear(choice.choiceKey);
                    } else {
    ...
    return (
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">{tc('characterBuilder.equipment.choiceLabel')}</p>
...
function BundlePackCard({ choiceKey, bundleId, contents, isSelected, icon: Icon, onSelect }: BundlePackCardProps) {
  const { t } = useTranslation('gamedata');
...
function formatItemLabel(itemId: string, quantity: number, t: GamedataT, explicitDef?: ItemDef): string {
