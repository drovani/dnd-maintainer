...
export function ProficienciesStep() {
  ...
  if (!resolved) {
    return <p className="text-muted-foreground text-sm">{tc('characterBuilder.proficiencies.selectClassFirst')}</p>;
  }
  ...
  function renderLanguageRow(lang: (typeof DND_LANGUAGE_DATA)[number]) {
    ...
    const speakersText =
      lang.typicalSpeakers.length > 0 ? lang.typicalSpeakers.map((s) => t(`creatureTypes.${s}`)).join(', ') : '—';
  ...
  const standardLangs = DND_LANGUAGE_DATA.filter(
    (l) => l.category === 'standard' && allLanguageIds.has(l.id as LanguageId)
  );
  const exoticLangs = DND_LANGUAGE_DATA.filter(
    (l) => l.category === 'exotic' && allLanguageIds.has(l.id as LanguageId)
  );
                  ...
                  <p className="text-sm text-muted-foreground">
                    ...
                    <span className="text-xs">
                      {tc('characterBuilder.pendingChoices.fromSource', {
                        source: getChoiceSourceName(tc_choice.choiceKey, t),
                      })}
                    </span>
                  </p>
                    ...
                    return (
                      ...
                      >
                        <Checkbox
                          ...
                          disabled={isDisabled}
                          onCheckedChange={(checked) => {
                            const next = checked ? [...selected, toolId] : selected.filter((id) => id !== toolId);
                            if (next.length === 0) {
              ...
              {standardLangs.length > 0 && (
                <>
                  <tr>
                    <th
                      colSpan={4}
                      className="pt-3 pb-1 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wider"
                    >
