...
export function Sidebar({
  ...
  return (
    <>
      ...
      {!isCollapsed && (
        <div className="fixed inset-0 bg-black/50 md:hidden z-30" onClick={() => onToggleCollapse(true)} />
      )}
      ...
      >
        ...
        <Link
          to="/"
          className="px-4 py-6 border-b border-sidebar-border flex items-center gap-3 hover:bg-sidebar-accent transition-colors"
        >
        ...
        <div className="px-4 py-4 border-b border-sidebar-border">
          ...
          <div className="relative">
            ...
            >
              ...
              {!isCollapsed && (
                <ChevronDown className={`size-4 transition-transform ${showCampaignDropdown ? 'rotate-180' : ''}`} />
              )}
            </button>
        ...
        <nav className="flex-1 px-2 py-4">
          {NAV_ITEMS.map((item) => {
            ...
            const fullPath = currentCampaign ? `/campaign/${currentCampaign.slug}${item.path}` : '#';
            ...
            const isActive = pathMatches(item.path) || (item.matchPaths?.some(pathMatches) ?? false);
