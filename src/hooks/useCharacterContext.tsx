...
export function CharacterProvider({
  ...
  const hasDeletedRows = useMemo(() => rows.some((r) => r.deleted_at != null && r.sequence !== 0), [rows]);
  ...
  const updateCreation = useCallback((updates: Readonly<CreationUpdates>) => {
    setRows((prev) => {
      ...
      const updated: BuildLevelRow = {
        ...
        ...(updates.choices !== undefined ? { choices: { ...(existing.choices ?? {}), ...updates.choices } } : {}),
      };
      const next = [...prev];
  ...
  const makeChoice = useCallback(
    (choiceKey: ChoiceKey, decision: ChoiceDecision) => {
      try {
        const next = [...rows];
  ...
  const levelUp = useCallback(
    (classId: ClassId, hpRoll: number | null, decisions?: ReadonlyMap<ChoiceKey, ChoiceDecision>) => {
  ...
  const replaceLevel = useCallback(
